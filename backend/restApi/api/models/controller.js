...
var apiURL = "https://labsdev.knolskape.com:8082"
                //////////////////
                //  RETRIEVE    //
                //////////////////
...
exports.getChatsIntegrated = function(req,res) {
  Log.find({"localTimeStamp":{"$gte":new Date(moment(req.query.enddate).subtract(10,'d').toISOString()), "$lte":new Date(moment(req.query.enddate).toISOString())}}, {"conversationId":1, "text":1, "localTimeStamp":1, "textId":1, "from":1, "inputHint":1, "userFeedback":1, "attachments":1, _id:0},function(err,item){
    if(err)
      res.send(err);
    res.json(item);
  });
...
exports.getGraphDataWithEnd = function(req,res) {
  ...
  Log.find({"localTimeStamp":{"$gte":new Date(moment(req.params.endDate).subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString()),"$lte":new Date(moment(req.params.endDate).toISOString())},"inputHint":"noAnswerFound"}).count(function(err,item){
    if(err)
      res.send(err);
    var noAnswer = item;
    Log.find({"localTimeStamp":{"$gte":new Date(moment(req.params.endDate).subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString()),"$lte":new Date(moment(req.params.endDate).toISOString())},"userFeedback":"No"}).count(function(err,item){
      if(err)
        res.send(err);
      var no = item;
      Log.find({"localTimeStamp":{"$gte":new Date(moment(req.params.endDate).subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString()),"$lte":new Date(moment(req.params.endDate).toISOString())},"userFeedback":"Yes"}).count(function(err,item){
        if(err)
          res.send(err);
        var yes = item;
        Log.find({"localTimeStamp":{"$gte":new Date(moment(req.params.endDate).subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString()),"$lte":new Date(moment(req.params.endDate).toISOString())}}).count(function(err,item){
            if(err)
                res.send(err);
            var total = item;
            var response = {noAnswer,no,yes,total};
...
exports.getGraphDataAggregated = function(req,res) {
      ...
      if(req.params.time ==="y")
      {
          ...
          //   Log.find({"localTimeStamp":{"$gte":new Date(moment().subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString()),"$lte":new Date(moment().toISOString())},"$or":[{"inputHint":"noAnswerFound"},{"userFeedback":"No"},{"userFeedback":"Yes"}]} ,function(err,item){
          //   if(err)
          //     res.send(err);

          //   var dateLimit2 = moment(JSON.parse(JSON.stringify(item))[0].localTimeStamp).add(1, 'months').startOf('month');
          //   var dateLimit2Clone = dateLimit2.clone();
          ...
          //   var j=initialMonth+(12-moment().month()-1);//start from initial month and readjust index
          ...
          //   //For Total Chats Aggregated, query count for each month
          //   // j=0;
          //   // var initialGte = moment().subtract(timediff.months,'M').subtract(timediff.days,'d').toISOString();
          //   // for (var i=0;i<12;i++){
          //   //     Log.find({"$and":[{"localTimeStamp":{"$gte":new Date(initialGte)}},{"localTimeStamp":{"$lte":new Date(dateLimit2Clone)}}]}).count(function(err,item){
          //   //         if(err)
          //   //             res.send(err);
          //   //         month[3][j] = item;
          //   //         console.log(item);
          //   //         initialGte=moment(initialGte).add(1,'M').toISOString();
          //   //         dateLimit2Clone=moment(dateLimit2Clone).add(1,'M').toISOString();
          //   //         if(i==11){
      ...
      else if(req.params.time ==="m")
      {
            ...
            for(var a=0;a<4;a++){
                labels.push(currenttimeClone.tz('Asia/Kolkata').format('Do YY'));
...
exports.getGraphDataAggregatedWithEnd = function(req,res) {
      ...
      else if(req.params.time ==="m")
      {
            ...
            var currenttimeClone = currenttime;
            for(var a=0;a<4;a++){
                labels.push(currenttimeClone.tz('Asia/Kolkata').format('Do MMM YY'));
                currenttimeClone.subtract(7,'d');
            }
      ...
      else if(req.params.time ==="d")
      {
            ...
            var dateLimit = moment(req.params.endDate).subtract(23,'h');
            var j=0;//start from 1st hour
